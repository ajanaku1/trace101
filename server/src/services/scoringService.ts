import type { WalletData, CategoryScore, RiskLevel } from '../types/exposure.js';
...
export function calculateSocialExposureScore(data: WalletData): CategoryScore {
  ...
  if (social.allDomains && social.allDomains.length > 0) {
    // Filter out duplicates with snsNames
  ...
  // Web3.bio additional profiles found
  if (social.web3BioProfiles && social.web3BioProfiles.length > 0) {
    // Count profiles not already captured above
    const knownPlatforms = ['twitter', 'x', 'farcaster', 'lens', 'ens', 'basenames', 'basename', 'discord', 'telegram', 'github'];
    const additionalProfiles = social.web3BioProfiles.filter(
      p => !knownPlatforms.some(k => p.toLowerCase().startsWith(k))
    );
    if (additionalProfiles.length > 0) {
      score += Math.min(additionalProfiles.length * 10, 30);
      signals.push(`Additional Web3 profiles found: ${additionalProfiles.length}`);
    }
  }
  ...
  // High-value wallet exposure
  if (data.solBalance > 100) {
    score += 10;
    signals.push('High-value wallet likely indexed by trackers');
  }
